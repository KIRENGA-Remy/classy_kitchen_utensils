...
import { login, register, googleAuth } from './auth.service';
...
export async function postGoogle(req: Request, res: Response, next: NextFunction) {
  try { res.json(await googleAuth(req.body)); } catch (err) { next(err); }
}
